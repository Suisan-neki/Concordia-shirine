...
- **内容を理解しない**: 会話の内容は一切解析・保存しません。音声の強さと時間パターンだけを分析します。
- **プライバシー重視**: すべての処理はブラウザ内で完結し、外部にデータを送信しません。
- **直感的な可視化**: 会話の状態を「波」の動きとして表現します。
...
#### 音声の解析
- マイクから取得した音声の**RMS（音声の強さ）**を計算
- 過去15秒間の平均エネルギーから「波の大きさ」を算出
- 音声/無音の切り替え回数や連続時間から、会話の状態を判定

#### 波の生成
- 複数のsin波を重ね合わせて波の形状を生成
- メイン波・サブ波・ノイズを組み合わせて自然な動きを表現
- シーンに応じて振幅・速度・周波数を調整
  - **調和**: 小さく穏やか（振幅0.7倍、速度0.5倍）
  - **一方的**: 大きく速く（振幅1.8倍、速度2.2倍）
  - **沈黙**: 大きく重く（振幅1.5倍、速度1.2倍）

#### 描画
- HTML5 Canvas APIで波の輪郭を描画
- グラデーションで色付け（シーンに応じて色も変化）
- 波頭に白い線を追加して立体感を表現
- 約60fpsでリアルタイムに再描画
...
**Q: AIを使っているの？**  
A: いいえ。音声の強さ（RMS）と時間パターンの分析のみです。機械学習やAIは使用していません。

**Q: 会話の内容を理解しているの？**  
A: いいえ。会話の内容は一切理解していません。音声の強さと、話している/いないの切り替えパターンだけを分析しています。

**Q: データは保存されるの？**  
A: いいえ。すべての処理はブラウザ内で完結し、音声データや解析結果は一切保存されません。

**Q: どうやって波を起こしているの？**  
A: 音声のエネルギーから「波の大きさ」を計算し、複数のsin波を重ね合わせて波の形を作っています。会話の状態に応じて、波の高さ・速度・色をリアルタイムに変更しています。

**Q: リアルタイムで動いているの？**  
A: はい。約60fpsで描画し、音声解析もリアルタイムで行っています。

**Q: どんな技術を使っているの？**  
A: Web Audio APIで音声取得、Canvas APIで描画、JavaScriptで計算処理を行っています。すべて標準的なWeb技術のみを使用しています。
